...
/*****************************************************************************************************************************************/
/*
int RBTree::compareRideNumbersInRBTree(RBNode* r1, RBNode* r2)
{
    if (r1->getRide()->getRideNumber() < r2->getRide()->getRideNumber())
    {
        return 0;
    }
    else if (r1->getRide()->getRideNumber() > r2->getRide()->getRideNumber())
    {
        return 1;
    }
    else
...
RBNode* RBTree::BSTInsert(RBNode* root, RBNode* node)
{
    ...
    int ret = compareRideNumbersInRBTree(node, root);
    if (ret == 0) // new node is smaller
    {
        root->left = BSTInsert(root->getLeftChild(), node);
        root->left->parent = root;
    }
    else if (ret == 1)
    {
        root->right = BSTInsert(root->getRightChild(), node);
        root->right->parent = root;
    }
    else if (ret == 2   )
    {
        std::cout << "Error: Duplicate Building IDs !" << std::endl;
        return NULL;
    }
...
{
    RBNode* parentPointer = NULL;
    RBNode* grandParentPointer = NULL;

    while ((node != root) && (node->color != BLACK) &&
        (node->getParentNode()->color == RED))
    {

        parentPointer = node->getParentNode();
        grandParentPointer = node->getParentNode()->getParentNode();

        //  Case : A
        //    Parent of pt is left child of Grand-parent of pt 
        if (parentPointer == grandParentPointer->getLeftChild())
        {

            RBNode* uncle_pt = grandParentPointer->getRightChild();
            ...
            else
            {
                ...
                if (node == parentPointer->getRightChild())
                {
                ...
                std::swap(parentPointer->color, grandParentPointer->color);
                node = parentPointer;
            }
        }
        ...
        {
            RBNode* uncle_pt = grandParentPointer->getLeftChild();
            ...
            else
            {
                ...
                //   pt is left child of its parent
                //   Right-rotation required 
                if (node == parentPointer->getLeftChild())
                {
...
// Code to imsert a new node in RBTree//
void RBTree::insertRideInRBTree(RBNode* node)
{
    // Do a normal BST insert
    this->RBTreeRoot = BSTInsert(this->RBTreeRoot, node);

    // fixing the Red Black Tree violations caused due to insert
    fixViolation(this->RBTreeRoot, node);
